...
public class EventRegistrationServiceImpl implements EventRegistrationService {
  ...
  public void importEventRegistration(MetaFile dataFile, Event event) {
    ...
    try {
      ...
      importEventRegistrationData(configXmlFile.getAbsolutePath(), tempDir, event);
  ...
  private File getConfigXmlFile() {
    ...
    try {
      ...
      InputStream bindFileInputStream =
          this.getClass().getResourceAsStream("/import-configs/csv-multi-config.xml");
  ...
  private void importEventRegistrationData(String configXmlFile, File tempDir, Event event) {
  ...
  public boolean checkEventRegistrationDate(EventRegistration eventRegistration) {
    ...
    Period startPeriod =
        Period.between(
            eventRegistration.getEvent().getRegOpenDate(),
            eventRegistration.getRegistrationDate().toLocalDate());
